...
branch seeds a galactic annulus in a gigayear without any spacecraft.  The
...
single civilisation could occupy the Galaxy in $\lesssim10^{8}$\,yr and that the
...
\citet{SaganNewman1983}, who held that no prudent civilisation would build a
self-reproducing probe because ``the probes would eventually become an
uncontrollable danger'' to their makers, a position Tipler rejected.
...
long it lasts.  Section~\ref{sec:record} compiles the observational record.
...
that afflicts any finite-fidelity copier \citep{Eigen1971,Gurzadyan2016}, and
predation or competition by other replicators \citep{Lotka2022}.  We treat the
...
system (Solar) & intact asteroid and Kuiper belts; no replicator population in the small-body catalogue (this repository's LOOM and DERELICT channels: 66\,686 Rubin orbits, all non-gravitational survivors explained) & no system-scale goo has occurred here & 4\,Gyr & yes \\
...
deliberate nanotechnology weapons; \citet{Ord2020} folds nanotechnology into
an ``unforeseen anthropogenic'' category at 1 in 30 per century and assigns no
separate value---the relevant $p_{\rm esc}$ is the fraction of
...
question is reached.  \citet{Gurzadyan2016} argue that finite-fidelity copying
...
to that row.  It assumes no competition: \citet{Lotka2022} show that
predator--prey dynamics between replicator strains can hold a population far
below the feedstock limit.  And the passive branch assumes a survival time
that no one has measured for anything but grains.
