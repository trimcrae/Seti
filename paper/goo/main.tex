...
such machines lands on other planets at a rate that does not depend on how far
the cloud has spread---of order \BeltArrivalsUnsurvived intact devices per
year on every rocky planet once it is phase-mixed around the solar annulus
($\sim$\AnnulusFillGyr\,Gyr), and the same total before---so functional
survival in interstellar space, which we scan from $10^{3}$\,yr to infinity,
sets not whether the devices land but how far ($v_\infty\tau$) and how fast a
chain of seedings advances: above a survival time of order the hop to the
nearest star ($\sim10^{5}$--$10^{6}$\,yr) the passive branch is a front at
$\sim$\SurvGrainVFront\kms that crosses the Galaxy in \SurvGrainTGalInf, without
any spacecraft; rock-borne devices never form one.  A
...
of covering fraction $f$ grinds itself down on a collisional time that ``is
roughly an orbital period divided by the covering fraction of the swarm'',
$\sim P_{\rm orb}/f$, so that a fully randomised swarm ``self-destructs within
a year or so''; the fragments then leave by Poynting--Robertson drag
...
The belt's mass in \SeedRadiusUm\um devices is \BeltDevicesReleased units; once
they are spread through the annulus volume an Earth-sized planet, whose
gravitationally focused cross-section at 26\kms is \FocusingFactor times its
geometric one, sweeps up \BeltArrivalsUnsurvived of them per year, and a
biosphere's mass ($10^{15}$\,kg) \BioArrivalsUnsurvived per year.

\subsection{What survival decides}
\label{sec:survival}

It is tempting to read the mixing time as a waiting time, so that a device
which stops working in less than \AnnulusFillGyr\,Gyr never lands anywhere.
That is wrong, and the reason organises the rest of this section.  A cloud of
$N_{c}$ carriers expanding at $v_\infty$ into a uniform field of stars of
density $n_{*}$, of which a fraction $\eta$ carry an Earth-sized planet, lands
on those planets at a total rate
\begin{equation}
\frac{dN_{\rm land}}{dt}=N_{c}(t)\,\eta\,n_{*}\,v_{\rm rel}\,\sigma ,
\label{eq:landrate}
\end{equation}
whatever volume the cloud currently fills: the carrier density falls exactly
as the number of enclosed targets rises.  Mixing decides when the rate is the
same at every planet, not when landings begin.  Survival therefore enters
only through $N_{c}(t)=N_{c}e^{-t/\tau}$, and its role is to set three other
things: the \emph{reach} $R=v_\infty\tau$ of a single release, the number of
targets inside it, and whether a \emph{chain} of seedings can propagate.  The
expected number of strict landings from one source over the window $W$ is
\begin{equation}
R_{0}(\tau)=N_{c}\,\eta\,n_{*}\,v_{\rm rel}\,\sigma\;\tau\left(1-e^{-W/\tau}\right),
\label{eq:r0tau}
\end{equation}
which is $N_{c}\eta n_{*}v\sigma W$ for an immortal device and falls only
linearly with $\tau$ below the window.  The first landing anywhere occurs at
$t_{\rm first}=\max(d_{1}/v_\infty,\,1/\dot N_{\rm land})$, where
$d_{1}=n_{*}^{-1/3}=\SurvDNearest$\,pc is the nearest-star distance; if
$R_{0}>1$ and $\tau>t_{\rm first}$ each seeded system re-emits and the chain
advances as a front at
\begin{equation}
v_{\rm front}\simeq\frac{v_\infty}{1+(t_{\rm first}+t_{\rm conv})\,v_\infty/d_{1}},
\end{equation}
the passive analogue of the inherited front's $v_f=d/(d/v+\tau_b)$
(Section~\ref{sec:active}) with the hop replacing the ship and the conversion
time of the next system's belt (\BeltConvTime) replacing the shipyard.

Table~\ref{tab:survival} and Figure~\ref{fig:survival} scan $\tau$ from
$10^{3}$\,yr to infinity for four carrier classes: the sub-micron devices of
this section, blown out at $v_\infty=18$\kms; micron-scale devices too large
for blowout, which leave at a few\kms by collisional and planetary scattering;
devices riding inside the $\sim10^{16}$ 100-m planetesimals a system ejects
during formation \citep{Do2018}; and devices inside the $6\times10^{10}$ rocks
a planet's surface loses to impacts over gigayears \citep{SirajLoeb2020grazing}.
For the sub-micron class the landing rate is $\dot N_{\rm land}=\SurvGrainRateZero$
per year over the whole Galaxy, the hop to the nearest star takes
\SurvGrainTHop, and so: with $\tau=10^{4}$\,yr the devices land on
$\SurvGrainRZeroTenKyr$ planets but all of them within \SurvGrainReachTenKyr\,pc,
which contains no star, so nothing is seeded; with $\tau=10^{5}$\,yr the reach
is \SurvGrainReachHundredKyr\,pc and \SurvGrainPlanetsHundredKyr\ planets, a
coin flip; with $\tau=10^{6}$\,yr the reach is \SurvGrainReachMyr\,pc,
\SurvGrainPlanetsMyr\ planets, and the chain propagates at
\SurvGrainVFront\kms, crossing the Galaxy in \SurvGrainTGalMyr---the same
crossing time as for an immortal device, because once $\tau$ exceeds the hop
time the front speed is set by the hop and the conversion, not by survival.
The transition from ``lands but seeds nothing'' to ``a galactic front'' is a
single decade wide and sits at $\tau\sim10^{5}$--$10^{6}$\,yr, three orders of
magnitude below the $\gtrsim10^{8}$\,yr a mixing-gated reading would demand
and well below the $\sim5\times10^{8}$\,yr on which supernova shocks destroy
interstellar silicate grains \citep{Jones1996}.  For scale, a 0.5\um sphere
outside the heliosphere intercepts \SurvGrainCrHits\ galactic-cosmic-ray
primaries per year, $\sim10^{5}$ over a $10^{6}$\,yr hop; whether a machine
tolerates that is a design question we do not prejudge, but it is the number
a design has to meet.  The micron dust-sized class behaves the same way with a
slower front (\SurvDustVFront\kms, \SurvDustTGalInf).  The rock-borne classes
do not: their carriers are so few that $R_{0}$ reaches unity only at
$\tau\to\infty$ for the planetesimals (\SurvIsoRZeroInf) and never for
planetary impact ejecta (\SurvRockRZeroInf), so shielding buys them survival
they cannot use.  Survival, in short, is the passive branch's threshold, not
its throttle: below $\sim10^{5}$\,yr the branch is a local scattering of
landings within a parsec, above $\sim10^{6}$\,yr it is a front, and the
crossing time above the threshold is \SurvGrainTGalInf\ regardless.

\begin{table}
\centering
\small
\caption{The survival scan.  For each carrier class the strict landings from
one source over 5\,Gyr ($R_{0}$), the reach $v_\infty\tau$ and the Earth-like
planets inside it, and the time for a chain of seedings to cross the Galaxy
($R_{\rm gal}=15$\,kpc), at survival e-folding times from $10^{4}$\,yr to
infinity.  Landings are counted whether or not the annulus has mixed
(eq.~\ref{eq:landrate}); a front requires $R_{0}>1$ and $\tau$ longer than the
time to the first landing.  $n_{*}=0.1$\,pc$^{-3}$, $\eta=0.1$.}
\label{tab:survival}
\begin{tabular}{lcccccc}
\toprule
$\tau$ (yr) & $10^{4}$ & $10^{5}$ & $10^{6}$ & $10^{7}$ & $10^{8}$ & $\infty$ \\
\midrule
\multicolumn{7}{l}{\emph{sub-micron devices, blown out} ($v_\infty=18$\kms; hop \SurvGrainTHop; $\dot N_{\rm land}=\SurvGrainRateZero$\,yr$^{-1}$)} \\
$R_{0}$ & \SurvGrainRZeroTenKyr & \SurvGrainRZeroHundredKyr & \SurvGrainRZeroMyr & \SurvGrainRZeroTenMyr & \SurvGrainRZeroHundredMyr & \SurvGrainRZeroInf \\
reach (pc) & \SurvGrainReachTenKyr & \SurvGrainReachHundredKyr & \SurvGrainReachMyr & \SurvGrainReachTenMyr & \SurvGrainReachHundredMyr & annulus \\
planets in reach & \SurvGrainPlanetsTenKyr & \SurvGrainPlanetsHundredKyr & \SurvGrainPlanetsMyr & \SurvGrainPlanetsTenMyr & \SurvGrainPlanetsHundredMyr & \SurvGrainPlanetsGyr \\
Galaxy crossed in & \SurvGrainTGalTenKyr & \SurvGrainTGalHundredKyr & \SurvGrainTGalMyr & \SurvGrainTGalTenMyr & \SurvGrainTGalHundredMyr & \SurvGrainTGalInf \\
\midrule
\multicolumn{7}{l}{\emph{micron dust-sized devices} ($v_\infty=5$\kms; hop \SurvDustTHop)} \\
$R_{0}$ & \SurvDustRZeroTenKyr & \SurvDustRZeroHundredKyr & \SurvDustRZeroMyr & \SurvDustRZeroTenMyr & \SurvDustRZeroHundredMyr & \SurvDustRZeroInf \\
planets in reach & \SurvDustPlanetsTenKyr & \SurvDustPlanetsHundredKyr & \SurvDustPlanetsMyr & \SurvDustPlanetsTenMyr & \SurvDustPlanetsHundredMyr & \SurvDustPlanetsGyr \\
Galaxy crossed in & \SurvDustTGalTenKyr & \SurvDustTGalHundredKyr & \SurvDustTGalMyr & \SurvDustTGalTenMyr & \SurvDustTGalHundredMyr & \SurvDustTGalInf \\
\midrule
\multicolumn{7}{l}{\emph{devices inside ejected 100-m planetesimals} ($10^{16}$ per system; first landing after \SurvIsoTFirst)} \\
$R_{0}$ & \SurvIsoRZeroTenKyr & \SurvIsoRZeroHundredKyr & \SurvIsoRZeroMyr & \SurvIsoRZeroTenMyr & \SurvIsoRZeroHundredMyr & \SurvIsoRZeroInf \\
Galaxy crossed in & $\infty$ & $\infty$ & $\infty$ & $\infty$ & $\infty$ & \SurvIsoTGalInf \\
\midrule
\multicolumn{7}{l}{\emph{devices inside impact-ejected rocks} ($6\times10^{10}$ per planet)} \\
$R_{0}$ & \SurvRockRZeroTenKyr & \SurvRockRZeroHundredKyr & \SurvRockRZeroMyr & \SurvRockRZeroTenMyr & \SurvRockRZeroHundredMyr & \SurvRockRZeroInf \\
Galaxy crossed in & $\infty$ & $\infty$ & $\infty$ & $\infty$ & $\infty$ & $\infty$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{fig3_survival}
\caption{The survival scan, $\tau$ from $10^{3}$\,yr to infinity.  Left: the
Earth-like planets within the reach $v_\infty\tau$ of one release, for the
blown-out class at 18\kms and every other carrier at 5\kms.  Centre: strict
landings from one source over 5\,Gyr, equation~(\ref{eq:r0tau}); the count
does not wait for phase mixing.  Right: the time for a chain of seedings to
cross the Galaxy; a front exists only above the hop time to the nearest star
and, for rock-borne devices, never on any useful timescale.}
\label{fig:survival}
\end{figure}
...
in rock, and the last two rows of Table~\ref{tab:survival} show what the
shielding is worth.  And a goo whose devices are larger than the blowout size
never leaves passively at all: dispersal class is decided at the micron.
...
all, provided $R_{0}>1$.  Functional survival with e-folding time $\tau$
replaces $W$ by $\tau(1-e^{-W/\tau})$, equation~(\ref{eq:r0tau}); landings do
not wait for mixing, so no factor $e^{-t_{\rm mix}/\tau}$ appears.
...
birth cluster (first 100\,Myr) & $10^{7}$--$3\times10^{16}$ bodies to the nearest sibling, by mechanism & --- & \ClusterRZeroStrict & \ClusterRZeroLoose & --- \\
\bottomrule
\end{tabular}
\end{table}
...
fewer than the $\sim10^{16}$ planetesimals a star ejects during formation
\citep{Do2018,AdamsNapier2022}.  \citet{SirajLoeb2020grazing} count
``20 per year of 30-cm objects penetrating the Earth's atmosphere and
eventually likely being ejected from the Solar System'', i.e.\ $6\times10^{10}$
over 3\,Gyr (95\% bounds $2\times10^{9}$--$3\times10^{11}$), and
\citet{AdamsNapier2022} quote 15 biologically active rocks of $10^{4}$\,g
ejected from an Earth-like planet per year; in the field these land on
\EjectaRZeroStrict\ other Earths per source.  \citet{AdamsNapier2022} reach the
same conclusion from the receiving end: a system captures $\sim70$ interstellar
rocks from the field over its life, and direct strikes on the Earth are
$10^{4}$ times rarer than capture by the system.  The planet-bound branch does
not leak into the Galaxy through impacts, as \citet{Melosh2003} concluded.  The
exception is bound capture: with a capture probability of $6.4\times10^{-7}$
per binary encounter and an encounter every $5\times10^{5}$\,yr,
$\sim2\times10^{8}$ ($10^{7}$--$10^{9}$) of those Earth-grazing bodies are
captured by binary systems over the Solar System's lifetime
...
siblings at kilometre-per-second relative speeds.  How many bodies cross
depends on the mechanism credited: weak (chaotic) transfer with a capture
probability of $1.5\times10^{-3}$ per body moves $10^{14}$--$3\times10^{16}$
bodies heavier than 10\,kg between the Sun and its nearest neighbour on
10-Myr timescales \citep{Belbruno2012}, while dynamical capture of the
$10^{16}$ rocks per star gives $\sim10^{7}$ captured from the cluster
\citep{AdamsNapier2022}; the odds of a given ejected meteoroid being recaptured
by another system are 1 in $10^{3}$--$10^{6}$, life-bearing rocks are captured
10--16\,000 times per cluster, and a fraction $f_{\rm imp}\sim10^{-4}$ of
captured rocks strike a terrestrial planet, giving $10^{-3}$--1.6
lithopanspermia events per cluster \citep{AdamsSpergel2005}; the Sun's own
Oort cloud was substantially captured from its siblings \citep{Levison2010}.
Taking the geometric middle of the transfer counts and the $10^{-4}$ landing
fraction, that is $R_{0}\approx\ClusterRZeroStrict$ strict touches per sibling
pair for rocks of any kind ($10^{3}$--$10^{12}$ across the range): it is likely
that the Earth has been struck by material that condensed around another
star.  A replicator, however, uses this channel only
...
the other extreme: at the measured flux of $1.5\times10^{-4}$\,m$^{-2}$\,s$^{-1}$
in the mass range $5\times10^{-18}$--$5\times10^{-16}$\,kg (Cassini at
0.7--1.2\,AU; \citealt{Kruger2019}; Ulysses and Galileo,
\citealt{Grun1993,Landgraf2000}), \DustGrainsPerYrOnEarth\ interstellar
...
ejects ($R_{0}\sim10^{-5}$).  The only channels far above threshold are the
dust-sized ones ($R_{0}\sim10^{19}$ for a converted belt), and those are the
ones whose carriers are least able to survive the crossing---but survival
cannot bring them below threshold: with a $10^{8}$\,yr functional lifetime the
belt's $R_{0}$ is \GrainRZeroFuncHundredMyr\ and with $10^{7}$\,yr
\GrainRZeroFuncTenMyr, because $R_{0}$ falls only linearly with $\tau$
(Section~\ref{sec:survival}).  What survival changes is the reach of each
landing and hence whether the chain has a front, and the threshold for that
is the hop time, $\sim10^{5}$--$10^{6}$\,yr.  For the interstellar-object
channel the carrier is a rock and survival is not the issue; the number is.
Contact, in other words, is never the bottleneck for a goo that is already in
a system's small bodies: what it reaches is set by whether its units are
grains or rocks, and how fast it reaches it by whether they work for longer
than a hop.  For a goo confined to a planet's surface, the graph confirms the
...
system (stars) & IRAS, 250\,000 sources \citep{Carrigan2009}; Gaia\,$\times$\,WISE: $<2\times10^{-5}$ of $2.7\times10^{5}$ stars within 100\,pc and $<8\times10^{-4}$ of $2.9\times10^{8}$ within 5\,kpc host 300\,K spheres at 90\% completion \citep{Suazo2022}; 7 candidates among 5 million sources \citep{Suazo2024}, every one since resolved into background galaxies \citep{Ren2024,Zackrisson2026} & no live swarm with $f\gtrsim0.1$ around $5\times10^{6}$ stars & live: indefinite; dead: $\leq P_{\rm orb}/f_{\min}$ & no \\
...
galaxy & \Ghat: WISE over $10^{5}$ galaxies \citep{Wright2014b,Griffith2015}; Tully--Fisher outliers among 1359 disc galaxies \citep{Zackrisson2015}; the mid-IR--radio correlation of the \Ghat sample \citep{Garrett2015} & ``no galaxies in our sample host an alien civilization reprocessing more than 85\% of its starlight''; fraction $<\GhatFractionBound$; $<0.3\%$ (tentative) to $<3\%$ (conservative) of disc galaxies star-fed Kardashev III & live: indefinite & no (beyond the reach horizon) \\
\bottomrule
\end{tabular}
\end{table}
...
elicited priors in the literature---\citet{Sandberg2008} report median
probabilities, by 2100, of 0.5\% for human extinction from ``the single biggest
nanotech accident'' and 5\% from ``molecular nanotech weapons'' (against 19\% for
extinction from all causes); \citet{Ord2020} assigns nanotechnology no
...
to that row.  It assumes no competition, and the one study of
competition argues the other way: \citet{Chen2022} model mutant ``predator''
probes that consume their ``prey'' siblings with Lotka--Volterra dynamics and
conclude that predation cannot sufficiently reduce the prey population, so
competition is not a reliable stop either.  And the passive branch and the
contact graph turn on a functional survival time that no one has measured for
anything but grains; we scan it from $10^{3}$\,yr to infinity rather than
assume it, and find that the answer changes character in one decade at
$\tau\sim10^{5}$--$10^{6}$\,yr and not at all above it.  For rock-borne
carriers the paper takes survival for granted and lets the number of carriers
decide.
...
a \SurvGrainTGalInf\ one for a passive chain of seedings.  Dispersal class,
decided at the micron and at the shipyard, sets the reach.
...
habitable zone.  Landings do not wait for the annulus to mix: the total
landing rate of an expanding cloud is independent of its volume.  Functional
survival in interstellar space sets the reach $v_\infty\tau$ and the front,
not the count: below $\tau\sim10^{5}$\,yr a release lands within a parsec and
seeds nothing, above $\tau\sim10^{6}$\,yr a chain of seedings advances at
$\sim$\SurvGrainVFront\kms and crosses the Galaxy in \SurvGrainTGalInf, the
same time as for an immortal device.  Rock-borne devices never form a chain
at any survival time.
...
($R_{0}\approx\IsoRZeroStrict$ by interstellar objects, $10^{-5}$ by impact
