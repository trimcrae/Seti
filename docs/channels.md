...
| **TOCSIN alt-feeds** (`tocsin.altfeeds`) | The same signature on non-Rubin feeds while Rubin is off sky | ASAS-SN Sky Patrol v2, ATLAS forced photometry | `tocsin-altfeeds-probe`, `tocsin-altfeeds-census`, `tocsin-altfeeds` | `tocsin-altfeeds.yml` | `tocsin-altfeeds.md`, `substitute-surveys.md` | `results/tocsin_altfeeds/census.json`: `OK` (139,706 targets); `probe.json`: `PARTIAL`. No screen committed yet |
