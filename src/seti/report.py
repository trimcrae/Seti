...
def collect_science_numbers(cfg: Config) -> dict[str, str]:
    ...
    out = {
        ...
        "SciNexcess": f"{counts.get('clean_excess', 0):,}".replace(",", r"\,"),
        "SciNexcessRaw": f"{counts.get('with_excess', 0):,}".replace(",", r"\,"),
...
def collect_laser_numbers(cfg: Config) -> dict[str, str]:
    """Macros for the laser-line paper from results/spectra/summary.json."""
    import json

    path = cfg.root / "results" / "spectra" / "summary.json"
    if not path.exists():
        return {}
    s = json.loads(path.read_text())
    occ = s.get("occurrence_limit", {})
    n_rej = sum(int(v) for v in s.get("rejection_counts", {}).values())
    ncand = int(s.get("n_candidates", 0))
    nspec = int(s.get("n_searched", 0))
    if ncand == 0:
        prose = ("No candidate survived the full funnel, a clean non-detection.")
    else:
        prose = (f"The {ncand} surviving candidates are isolated, unresolved, "
                 "non-recurrent emission lines retained for higher-resolution "
                 "spectroscopic follow-up.")
    return {
        "LaserNspec": f"{nspec:,}".replace(",", r"\,"),
        "LaserNrejected": f"{n_rej:,}".replace(",", r"\,"),
        "LaserNcand": f"{ncand:,}".replace(",", r"\,"),
        "LaserFupper": _fmt_sci(occ.get("f_upper", float("nan")), 1),
        "LaserDataset": str(s.get("dataset", "DESI-DR1")).replace("_", r"\_"),
        "LaserSpectype": str(s.get("spectype", "all")),
        "LaserSnrMin": "8",
        "LaserResultProse": prose,
    }
...
def write_laser_numbers_tex(cfg: Config) -> Path:
    macros = collect_laser_numbers(cfg)
    lines = ["% Auto-generated by `seti laser-numbers`. Do not edit by hand."]
    for key, val in macros.items():
        # ResultProse is a full sentence; the rest get \xspace.
        suffix = "" if key == "LaserResultProse" else r"\xspace"
        lines.append(rf"\newcommand{{\{key}}}{{{val}{suffix}}}")
    out = cfg.root / "paper" / "laser" / "numbers.tex"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text("\n".join(lines) + "\n")
    return out


__all__ = ["collect_numbers", "write_numbers_tex", "collect_laser_numbers",
           "write_laser_numbers_tex"]
