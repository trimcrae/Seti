...
\begin{tikzpicture}[scale=3.2]
  ...
  \coordinate (X4) at (0.4,0.85);

% AXES
  \draw[->,thick] (0,-1.15) -- (0,1.15) node[above=1,left=-1] {$t''$};
  \draw[->,thick] (-2.15,0) -- (2.15,0) node[left=1,above=0] {$x''$};
  ...
  \begin{scope}
    ...
    \foreach \i [evaluate={\c=\i/(\Nlines+1); \cs=sin(90*\c);}] in {1,...,\Nlines}{
      ...
      \draw[world line,samples=\Nsamples,smooth,variable=\x,domain=0:2] % region II/IV, constant r
        ...
        %plot(\x-1,{1-3.2*kruskal(\x*pi/4,\cs-0.6)});
        plot(\x-1,{1-2.2*kruskal(\x*pi/4,\cs-0.75)});
  ...
  \path (S) -- (W) node[mydarkblue,pos=0.50,below=-2.5,rotate=-45,scale=0.85]
    {\contour{mylightpurple}{anti-horizon $r=2M$}};
  \path (W) -- (N) node[mydarkblue,pos=0.32,above=-2.5,rotate=45,scale=0.85]
    {\contour{mylightpurple}{horizon $r=2M$}};
  ...
  % INFINITY LABELS
  \node[above=1,left=1,mydarkblue] at (-2.15,0) {$i^0$};
  ...
  \node[fill=mylightpurple,inner sep=2] at (0.2,0.88) {$\psi(U)$};
  \node[fill=mylightpurple,inner sep=2] at (0.3,-0.6) {$\psi(p)$};
  %\node[fill=mylightpurple,inner,sep=2] at (0.2,0.5) {$\psi(U_1)$};
  % LIGHT CONE FRONT
  %\conefront{X4};
