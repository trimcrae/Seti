...
| **ROMAN/lens** (`roman.lens`) | S40, the opaque lens: a microlens opaque over a fraction ρ_L of its Einstein radius removes the minor image in the wings (u > 1/ρ_L − ρ_L), a symmetric pair of steps of depth A₋(u_c); implied density below any bound body | GBTDS Level 4 light curves (F146 + F087); pre-launch: the RMDC26 challenge light curves (Hugging Face `RGES-PIT/*`, W149/Z087) | `roman screen --channel lens` | `roman.yml` | `roman.md` §2.1, §4A | `results/roman/lens/summary.json` (run 34349717932): `SIMULATION_PACES_OK`; 1,262 of 2,688 simulated supernovae preferred the occulting model before the time-symmetry gate existed; with it (run 4) 2,511 are `NOT_LENSING` and 145 stay pending only because their peak sits at the coverage edge. Prior art: the occulting-lens morphology is natural-body literature (Agol 2002; arXiv:2608.24009), the density test and the search are not. Pending flight data |
...
| **ROMAN/paces** (`roman.bridge`) | Every existing light-curve / spectrum channel (dips, glint, secular fade, RUST, KNELL, METRONOME, narrow lines) on Roman products with GBTDS-season parameters and the F146/F087 achromaticity test | as above | `roman screen --channel paces` | `roman.yml` | `roman.md` §3, §4A | `results/roman/paces/summary.json` (run 4): `SIMULATION_PACES_OK` on 2,688 simulated SN Ia curves (daily SIMLIB sampling) — 1,205 tagged transient-like with their dip/glint flags counted apart; secular/RUST/KNELL `insufficient` on all; glint is `not_applicable` above a 0.5-day cadence. Not a sky result |
