...
``probe``     HEAD/GET every registered bulk-download route; no download.
              Writes ``probe.json`` so the next dispatch knows which host
              answers before spending a 760 MB pull on it.
``acquire``   pull GALAH DR4 (optionally APOGEE DR17) through
              ...
              ``acquisition.json`` (the acquisition log + columns found).
``screen``    per survey and per sample (dwarf primary, giant secondary):
              peer residuals, the five-hypothesis fit for every star, the
              raw-space fit, the no-Ba fit, and leave-one-out for every star
              above the prefilter. Checkpoints ``scores_*.parquet`` and
              ``vectors_*.parquet``.
``assess``    population-level calibration: the whole-sample and
              shuffled-element null distributions, the working threshold,
              the vetoes at that threshold, the sensitivity curve, then
...
CHANNEL = "fallout"
STAGES: tuple[str, ...] = ("probe", "acquire", "screen", "assess", "all")
...
def _pattern_config(block: dict) -> P.PatternConfig:
    ...
    for key in ("elements", "core_elements"):
        if key in kw:
            kw[key] = tuple(kw[key])
...
def screen_sample(stars: pd.DataFrame, *, survey: str, sample: str, block: dict,
                  out_dir: Path) -> dict:
    """Score one (survey, sample): peer residuals, fits, LOO. Checkpoints to disk."""
    ...
    # Peer-space vectors: residual as the value, catalogue error + floor as sigma,
    # flags from the canonical table.
    frame = stars.copy()
    for el in elements:
    ...
    # Leave-one-out only where it can matter.
    ...
    keep = [c for c in ("star_id", "ra", "dec", "teff", "logg", "fe_h", "snr", "flag_sp", "age",
                        "binary_flag", "ruwe", "vsini", "chi2_sp", "survey") if c in stars.columns]
...
def assess_sample(rec: dict, *, block: dict, out_dir: Path) -> dict:
    ...
    raw_pass = np.isfinite(scores["fission_lr_raw"].to_numpy(dtype=float)) \
        & (scores["fission_lr_raw"].to_numpy(dtype=float) >= thr)
    ...
    sens = P.sensitivity_curve(obs, sig, T, cfg, lr_threshold=thr,
                               ...
                               n_inject=int(sb.get("n_inject", 1500)), rng=rng)
    n_show = int((block.get("report") or {}).get("max_candidates", 50))
    survivors = cand[cand["vet_pass"]] if len(cand) else cand
    show_cols = [c for c in ["star_id", "teff", "logg", "fe_h", "snr", "n_measured", "fission_lr",
                             "enrich_lr", "lr_noba", "lr_loo_min", "lr_loo_driver", "fission_lr_raw",
                             "a_f", "classification", "natural_class", "first_veto", "veto_reasons"]
                 + list(elements) if c in (cand.columns if len(cand) else [])]
    return {
        **rec,
        "threshold": {"lr_used": thr, "why": thr_why, "lr_min_config": cfg.lr_min,
                      "enrich_min": cfg.enrich_min},
        ...
        "sensitivity": sens,
        "survivors": survivors[show_cols].head(n_show).to_dict(orient="records") if len(survivors) else [],
        "vetoed_top": cand[~cand["vet_pass"]][show_cols].head(10).to_dict(orient="records") if len(cand) else [],
        "verdict": None,
    }


def _overall_verdict(acq: dict | None, per_sample: list[dict]) -> tuple[str, str]:
    ...
    n_prim = sum(int(s.get("n_survivors", 0)) for s in prim)
    n_sec = sum(int(s.get("n_survivors", 0)) for s in scored if s not in prim)
    ...
    if code == VERDICT_NO_PATTERN:
        text = (prefix + f"{VERDICT_NO_PATTERN}: no star in any sample kept the fission-only "
                "preference through the vetoes at the null-calibrated threshold. Per CLAUDE.md this "
                "is a reason to change the question (a second survey, the APOGEE Ce/Nd panel, "
                "differential co-natal pairs), not a result to write up.")
    else:
        text = (prefix + f"{VERDICT_CANDIDATES}: {n_prim} cool-dwarf and {n_sec} giant "
                "(lower-weight) catalogue-level survivors. None is a detection until the "
                "n-capture lines are re-measured from the raw spectra against Teff-matched peers "
                "and the pattern is confirmed element by element.")
    if degraded and code == VERDICT_NO_PATTERN and prefix:
...
def stage_assess(block: dict, out_dir: Path, surveys: list[str], screen: list[dict] | None,
    ...
    if screen is None:
        # Re-assessing a prior screen: rebuild the per-sample records from disk.
        screen = []
        for sv in surveys:
    ...
    per_sample = [assess_sample(r, block=block, out_dir=out_dir) if r.get("sample") else r
                  for r in screen]
    cfg = _pattern_config(block)
    code, text = _overall_verdict(acq, per_sample)
    ...
    funnel = {
        ...
        "per_sample": [{"survey": s.get("survey"), "sample": s.get("sample"),
                        ...
                        "n_survivors": int(s.get("n_survivors", 0))} for s in per_sample],
    ...
    summary = {
        ...
        "verdict": text,
        "funnel": funnel,
        "vetoes": {f"{s.get('survey')}/{s.get('sample')}": s.get("vetoes") for s in per_sample if s.get("vetoes")},
        "acquisition": acq,
        "columns_found": {s["survey"]: s.get("columns_found") for s in ((acq or {}).get("surveys") or [])},
        "templates": templates,
        "thresholds": {f"{s.get('survey')}/{s.get('sample')}": s.get("threshold") for s in per_sample if s.get("threshold")},
        "per_sample": per_sample,
    }
    _json(out_dir / "summary.json", summary)
    write_report(out_dir, summary)
    print("[fallout] " + json.dumps({"verdict": code, **{k: v for k, v in funnel.items() if k != "per_sample"}}))
    return summary
...
def write_report(out_dir: Path, summary: dict) -> Path:
    ...
    L.append("Fission product, once its short-lived members have decayed, is a fixed *shape* in "
             ...
             "is only a candidate if removing any one element leaves the preference standing.\n")
    ...
    for s in acq.get("surveys") or []:
        ...
        if cf:
            L.append(f"  - pattern elements found: {', '.join(cf.get('elements') or [])}")
    ...
    for s in summary.get("per_sample") or []:
        L.append(f"## {s.get('survey')} / {s.get('sample')}\n")
        ...
        L.append(f"- threshold: ln LR ≥ **{th.get('lr_used', float('nan')):.2f}** ({th.get('why')}); "
                 f"enrichment ln LR ≥ {th.get('enrich_min')}")
        ...
        if sens:
            ...
            L.append("| a_f | Δ[Nd/H] dex | LR pass | LR + leave-one-out pass |")
            L.append("|---|---|---|---|")
            for r in sens:
                L.append(f"| {r['a_f']:g} | {r['nd_dex']:+.2f} | {r['frac_lr_pass']:.2f} | "
                         f"{r.get('frac_lr_and_loo_pass', float('nan')):.2f} |")
        sv = s.get("survivors") or []
        if sv:
            L.append("\n### Survivors (catalogue-level; pending re-measurement)\n")
            L.append("| star | Teff | log g | [Fe/H] | n_el | ln LR | no-Ba | LOO min (driver) | raw | a_f |")
            L.append("|---|---|---|---|---|---|---|---|---|---|")
            for c in sv[:25]:
                L.append(f"| {c.get('star_id')} | {float(c.get('teff', np.nan)):.0f} | "
                         ...
                         f"({c.get('lr_loo_driver')}) | {float(c.get('fission_lr_raw', np.nan)):.1f} | "
...
def fallout_run(cfg: Config | None = None, *, stage: str = "all", surveys: str | None = None,
    ...
    return stage_assess(block, out, sv_list, screen, acq, stage)
